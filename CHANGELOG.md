...
- `DefaultVertexFormat.POSITION_COLOR_TEX` and `GameRenderer::getPositionColorTexShader` do not exist in MC 1.21.1; corrected to `POSITION_TEX_COLOR` and `getPositionTexColorShader` (tex before color) across all three renderers; vertex attribute call order also swapped to `.setUv().setColor()` to match the format's element layout
